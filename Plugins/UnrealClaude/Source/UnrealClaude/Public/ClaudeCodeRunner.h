...
public:
	...
	/** Check if the runner is configured to use Codex CLI. */
	static bool IsUsingCodexCli();

	/** Get the Codex CLI path. */
	static FString GetCodexPath();

	/** Get the optional Codex JS CLI entrypoint path. */
	static FString GetCodexScriptPath();
...
private:
	...
	FString BuildCodexCommandLine(const FClaudeRequestConfig& Config);
	FString BuildCodexPromptPayload(const FString& TextPrompt) const;
	FString ParseCodexExecOutput(const FString& RawOutput) const;
	void ExecuteProcess();
	void CleanupHandles();
...
private:
	...
	FString CodexLastMessageFilePath;
