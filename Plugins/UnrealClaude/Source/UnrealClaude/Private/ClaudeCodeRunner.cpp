...
bool FClaudeCodeRunner::IsUsingCodexCli()
{
	if (const UUnrealClaudeSettings* Settings = GetDefault<UUnrealClaudeSettings>())
	{
		return Settings->bUseCodexCli;
	}

	return false;
}

FString FClaudeCodeRunner::GetCodexPath()
{
	// Cache the path to avoid repeated lookups and log spam
	...
	if (const UUnrealClaudeSettings* Settings = GetDefault<UUnrealClaudeSettings>())
	{
		if (!Settings->CodexCliPath.IsEmpty() && IFileManager::Get().FileExists(*Settings->CodexCliPath))
		{
			CachedCodexPath = Settings->CodexCliPath;
	...
	FString EnvCodexCliPath = FPlatformMisc::GetEnvironmentVariable(TEXT("CODEX_CLI_PATH"));
	if (!EnvCodexCliPath.IsEmpty())
	{
		PossiblePaths.Add(EnvCodexCliPath);
	}
	...
	if (!UserProfile.IsEmpty())
	{
		PossiblePaths.Add(FPaths::Combine(UserProfile, TEXT("AppData"), TEXT("Local"), TEXT("OpenAI"), TEXT("Codex"), TEXT("bin"), TEXT("codex.exe")));
	}
	...
	// Scan every PATH directory for codex.cmd or codex.exe as a last resort
	FString PathEnv = FPlatformMisc::GetEnvironmentVariable(TEXT("PATH"));
	TArray<FString> PathDirs;
	PathEnv.ParseIntoArray(PathDirs, TEXT(";"), true);
	...
	if (FPlatformProcess::ExecProcess(WhichCmd, WhichArgs, &ReturnCode, &WhereOutput, &WhereErrors) && ReturnCode == 0)
	{
		...
		if (Lines.Num() > 0)
		{
			UE_LOG(LogUnrealClaude, Log, TEXT("Found Codex CLI via '%s': %s"), WhichCmd, *Lines[0]);
			CachedCodexPath = Lines[0];
			return CachedCodexPath;
		}
	}

	UE_LOG(LogUnrealClaude, Warning, TEXT("Codex CLI not found. Install Codex or set Project Settings -> Plugins -> Unreal Claude -> Codex CLI Path."));

	// CachedCodexPath remains empty if not found
	return CachedCodexPath;
}

FString FClaudeCodeRunner::GetCodexScriptPath()
{
	if (const UUnrealClaudeSettings* Settings = GetDefault<UUnrealClaudeSettings>())
	{
		if (!Settings->CodexCliScriptPath.IsEmpty() && IFileManager::Get().FileExists(*Settings->CodexCliScriptPath))
		{
			return Settings->CodexCliScriptPath;
		}
	}
	...
	FOnClaudeProgress OnProgress)
{
	...
	{
		bIsExecuting = false;
		OnComplete.ExecuteIfBound(IsUsingCodexCli()
			? TEXT("Codex CLI not found. Install Codex or set Project Settings -> Plugins -> Unreal Claude -> Codex CLI Path.")
			: TEXT("Claude CLI not found. Please install with: npm install -g @anthropic-ai/claude-code"), false);
		return false;
	}
...
bool FClaudeCodeRunner::ExecuteSync(const FClaudeRequestConfig& Config, FString& OutResponse)
{
	if (!IsClaudeAvailable())
	{
		OutResponse = IsUsingCodexCli()
			? TEXT("Codex CLI not found. Install Codex or set Project Settings -> Plugins -> Unreal Claude -> Codex CLI Path.")
			: TEXT("Claude CLI not found. Please install with: npm install -g @anthropic-ai/claude-code");
		return false;
	}
	...
	UE_LOG(LogUnrealClaude, Log, TEXT("Executing agent CLI: %s %s"), *ClaudePath, *CommandLine);
...
FString FClaudeCodeRunner::BuildCommandLine(const FClaudeRequestConfig& Config)
{
	if (IsUsingCodexCli())
	{
		return BuildCodexCommandLine(Config);
	}
...
FString FClaudeCodeRunner::BuildCodexCommandLine(const FClaudeRequestConfig& Config)
{
	FString CommandLine;

	const FString CodexScriptPath = GetCodexScriptPath();
	if (!CodexScriptPath.IsEmpty())
	{
		CommandLine += FString::Printf(TEXT("\"%s\" "), *FPaths::ConvertRelativePathToFull(CodexScriptPath));
	}

	CommandLine += TEXT("exec --json ");
	...
	if (const UUnrealClaudeSettings* Settings = GetDefault<UUnrealClaudeSettings>())
	{
		if (!Settings->CodexModelProviderOverride.IsEmpty())
		{
			CommandLine += FString::Printf(TEXT("-c model_provider=%s "), *Settings->CodexModelProviderOverride);
		}

		if (!Settings->CodexModelOverride.IsEmpty())
		{
			CommandLine += FString::Printf(TEXT("--model \"%s\" "), *Settings->CodexModelOverride);
		}
	}
	...
	for (const FString& ImagePath : Config.AttachedImagePaths)
	{
		...
		{
			CommandLine += FString::Printf(TEXT("--image \"%s\" "), *FPaths::ConvertRelativePathToFull(ImagePath));
		}
	}
...
FString FClaudeCodeRunner::BuildCodexPromptPayload(const FString& TextPrompt) const
{
	return TextPrompt;
}

FString FClaudeCodeRunner::ParseCodexExecOutput(const FString& RawOutput) const
{
	TArray<FString> Lines;
	...
	for (const FString& Line : Lines)
	{
		...
		FString Message;
		if (JsonObj->TryGetStringField(TEXT("message"), Message) && !Message.IsEmpty())
		{
			LastResultText = Message;
		}

		const TSharedPtr<FJsonObject>* MsgObj = nullptr;
		if (JsonObj->TryGetObjectField(TEXT("msg"), MsgObj) || JsonObj->TryGetObjectField(TEXT("message"), MsgObj))
		{
			FString Content;
		...
		if (Type.Contains(TEXT("assistant"), ESearchCase::IgnoreCase))
		{
			FString Text;
...
bool FClaudeCodeRunner::LaunchProcess(const FString& FullCommand, const FString& WorkingDir)
{
	...
	UE_LOG(LogUnrealClaude, Log, TEXT("Agent process started successfully"));
	...
	while (!StopTaskCounter.GetValue())
	{
		...
		if (!OutputChunk.IsEmpty())
		{
			...
			while (NdjsonLineBuffer.FindChar(TEXT('\n'), NewlineIdx))
			{
				...
				if (!CompleteLine.IsEmpty() && !IsUsingCodexCli())
				{
		...
		if (!FPlatformProcess::IsProcRunning(ProcessHandle))
		{
			...
			if (!NdjsonLineBuffer.IsEmpty() && !IsUsingCodexCli())
			{
...
void FClaudeCodeRunner::ExecuteProcess()
{
	...
	UE_LOG(LogUnrealClaude, Log, TEXT("Async executing agent CLI: %s %s"), *ClaudePath, *CommandLine);
	...
	if (StdInWritePipe)
	{
		...
		if (IsUsingCodexCli())
		{
			if (!CurrentConfig.SystemPrompt.IsEmpty())
			{
				TextPrompt = FString::Printf(TEXT("[CONTEXT]\n%s\n[/CONTEXT]\n\n"), *CurrentConfig.SystemPrompt);
			}
			TextPrompt += CurrentConfig.Prompt;
		}
		else if (!SystemPromptFilePath.IsEmpty())
		{
		...
		if (!IsUsingCodexCli() && !PromptFilePath.IsEmpty())
		{
		...
		FString StdinPayload = IsUsingCodexCli()
			? BuildCodexPromptPayload(TextPrompt)
			: BuildStreamJsonPayload(TextPrompt, CurrentConfig.AttachedImagePaths);
		// Cache the stdin payload for the silence watchdog diagnostic.
		LastStdinPayload = StdinPayload;

		if (!StdinPayload.IsEmpty())
		{
			UE_LOG(LogUnrealClaude, Log, TEXT("Writing prompt to agent stdin (%d chars)"), StdinPayload.Len());
			...
			UE_LOG(LogUnrealClaude, Log, TEXT("Wrote to agent stdin (success=%d, %d/%d bytes, images: %d, system: %d chars, user: %d chars)"),
		...
		{
			UE_LOG(LogUnrealClaude, Warning, TEXT("Agent stdin payload was empty"));
		}
	...
	if (IsUsingCodexCli())
	{
		if (!CodexLastMessageFilePath.IsEmpty())
		{
			TArray<uint8> ResponseBytes;
			if (FFileHelper::LoadFileToArray(ResponseBytes, *CodexLastMessageFilePath) && ResponseBytes.Num() > 0)
			{
				ResponseBytes.Add(0);
				ResponseText = FString(UTF8_TO_TCHAR(reinterpret_cast<const char*>(ResponseBytes.GetData())));
			}
		}

		if (ResponseText.IsEmpty() && !FullOutput.IsEmpty())
		{
			ResponseText = ParseCodexExecOutput(FullOutput);
		}
	}
	else
	{
	...
	CodexLastMessageFilePath.Empty();
